...
def do_search(query=None, sort="0", pub="0", offset="0", search_id=""):
    ...
    lives = [row for row in items if (row or {}).get("kind") == "live" or (row or {}).get("room_id")]
    videos = [
        row
        for row in items
        if (row or {}).get("kind") not in ("user", "live") and not (row or {}).get("room_id")
    ]
    if sort not in ("0", ""):
        videos = sort_videos(videos, sort)
    videos = filter_by_publish(videos, pub)
    if not users and not lives and not videos:
        notify("这一页没有视频了" if offset else "没搜到。换个全名再试，或确认已登录")
    ...
    if users and offset <= 0:
        names = "、".join(
            str(row.get("author") or row.get("nickname") or "") for row in users[:6] if row.get("author") or row.get("nickname")
        )
        add_dir(
            "用户（%s）" % len(users),
            {
...
def do_search_users(query="", sort="0", pub="0", offset="0", search_id=""):
    ...
    cached = load_search_cache(PROFILE, query, sort, pub, int(offset or 0) if str(offset or "0").isdigit() else 0)
    items = (cached or {}).get("items") or []
    users = [row for row in items if (row or {}).get("kind") == "user"]
    if not users:
        try:
            items, _more, _off, _sid = client().search_page(
                query, sort_type=sort, publish_time=pub, offset=0, search_id=search_id
            )
        except Exception:
            items = []
        users = [row for row in items if (row or {}).get("kind") == "user"]
